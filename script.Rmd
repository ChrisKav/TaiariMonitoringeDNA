...
main_env <- subset(env, Type == "Main") %>%
  select(Site, Latitude, Longitude, Temp, Cond, CUM_AREA, LENGTHDOWN, upElev, segslpmean, LANDCOVER) %>%
  arrange(LENGTHDOWN)
...
main_env$segslpmean <- round(main_env$segslpmean, 2)
main_env$Temp <- round(main_env$Temp, 1)

#write.table(main_env, file = "main_env.txt", sep = ",", quote = FALSE, row.names = F)

trib_env <- subset(env, Type == "Trib") %>%
  arrange(`SubCatch`, LENGTHDOWN) %>%
  select(Site, Latitude, Longitude, Temp, Cond, CUM_AREA, LENGTHDOWN, upElev, segslpmean, LANDCOVER,`SubCatch`)
...
trib_env$segslpmean <- round(trib_env$segslpmean, 2)
trib_env$Temp <- round(trib_env$Temp, 1)

#write.table(trib_env, file = "trib_env.txt", sep = ",", quote = FALSE, row.names = F)
```
